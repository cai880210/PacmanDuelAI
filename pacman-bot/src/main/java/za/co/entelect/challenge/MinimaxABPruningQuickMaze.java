...
public class MinimaxABPruningQuickMaze {
    ...
    private static final int default_maxdepth = 9;
    ...
    private static int alphabeta(QuickMaze node, int depth, int alpha, int beta, boolean maximizingPlayer) {
        if (depth == default_maxdepth && maximizingPlayer) { // OR NODE IS TERMINAL; IE: NO PILLS LEFT. VERY IMPORTANT. SAVE A LOT OF PROCESSING POWER BRUV
                                                                //                ^---- stop evaluating the path if there is nothing left. 
            return node.getPointValue();
        }
        if (maximizingPlayer) {
            for (QuickMaze child: node.getChildNodes(true)) {
            ...
            for (QuickMaze child: node.getChildNodes(false)) {
                beta = Math.min(beta, alphabeta(child, depth, alpha, beta, true));
                if(beta <= alpha) {
                    break;
                }
            }
